...
const components = {
  ...
  pre: ({ className, ...props }: ComponentsProps) => (
    <pre
      className={cn(
        "mb-4 mt-6 overflow-x-auto text-sm  rounded-lg border dark:!bg-secondary py-4",
        className,
      )}
      {...props}
    />
  ...
  code: ({ className, ...props }: ComponentsProps) => (
    <code
      className={cn(
        "relative rounded border px-[0.3rem] py-[0.2rem] dark:!bg-secondary font-mono font-light !text-sm",
        className,
      )}
      {...props}
    />
